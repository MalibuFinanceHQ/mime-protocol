...
import { AaveManipulator, CheckERC20ApproveSpenderManipulator, CopyTrader, CopyTrader__factory, ReplaceERC20ApproveSpenderManipulator } from '../typechain';
...
import { deployManipulators } from './utils/deployManipulators';
...
describe('Relay Transaction: test', function () {
    ...
    before(async () => {
        ...
        const proxy = await (<CopyTrader__factory>(
        ...
        erc20ApproveReplaceSenderManipulator = manipulators.erc20ApproveReplaceSenderManipulator;
        erc20ApproveSpendersWhitelist = manipulators.erc20ApproveSpendersWhitelistManipulator;
    ...
    step("Should deploy and setup a strategy", async () => {
        ...
        await tradingStrategy.setManipulator(mockDAI.address, approveMethodSig, erc20ApproveSpendersWhitelist.address);

        // Setup AAVE deposit manipulator.
        const depositMethodSig = utils.arrayify('0xe8eda9df');
        await tradingStrategy.setManipulator(mockLendingPool.address, depositMethodSig, aaveDepositManipulator.address);
    });
